...
install-deb: set-user-alarm
	sudo checkinstall "--requires=systemd, pulseaudio-utils, gnome-session-canberra, python3-psutil" --pkgname=wake-mobile --pkglicense=GPL-2+ --nodoc --pkgversion=1.4 --pkgrelease=0 --include=listfile --deldesc=yes --backup=no -y
